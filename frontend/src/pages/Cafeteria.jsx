...
// Medios de pago rápidos para la cafetería (efectivo/tarjeta/QR). El fiado, si hace falta, se cobra en Caja.
const MEDIOS = [
...
export default function Cafeteria() {
  ...
  const cargarProductos = () =>
    api.platos({}).then((ps) => setProductos(ps.filter((p) => p.cat_cafeteria && p.activo !== 0)));
  const cargarMesas = () => api.cafeteriaMesas().then(setMesas).catch(() => {});
  ...
  const nuevaMesa = async () => {
    try {
      ...
      await cargarMesas();
      setSelId(p.id); setCobrando(false);
  ...
  // Ticket opcional (solo cuando el cliente lo necesita; el cobro normal no imprime)
  ...
  const cobrar = async (medio) => {
    ...
    try {
      ...
      setCobrando(false); setSelId(null);
      await cargarMesas();
      toast('✅ Cobrado.');
    } catch (e) {
  ...
  return (
    <div>
      <div style={{ display: 'flex', gap: 8, alignItems: 'center', marginBottom: 12, flexWrap: 'wrap' }}>
        ...
        <span style={{ color: 'var(--muted)', fontSize: 13 }}>Mesas abiertas:</span>
        {mesas.map((m, i) => (
          <button key={m.id} className={'chip' + (m.id === selId ? ' active' : '')}
            onClick={() => { setSelId(m.id); setCobrando(false); }} style={{ fontWeight: 700 }}>
            Mesa {i + 1} · {money(m.total)}
          </button>
        ))}
      ...
      <div className="grid" style={{ gridTemplateColumns: '1fr 340px', gap: 16, alignItems: 'start' }}>
        ...
        <div className="card">
          ...
          {sel && (
            <>
              <h2 className="h2" style={{ marginTop: 0 }}>🧾 Mesa · {money(sel.total)}</h2>
              ...
              {!cobrando ? (
                <>
                  <button className="btn-green" style={{ width: '100%', padding: 13, marginTop: 8 }}
                    disabled={!items.length} onClick={() => setCobrando(true)}>💵 Cobrar {money(sel.total)}</button>
